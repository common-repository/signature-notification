...
namespace BracketSpace\Notification\Signature\Dependencies\BrianHenryIE\Strauss\Composer\Extra;

use BracketSpace\Notification\Signature\Dependencies\Composer\Composer;
use Exception;
use BracketSpace\Notification\Signature\Dependencies\JsonMapper\JsonMapperFactory;
...
class StraussConfig
{
    ...
    public function __construct(Composer $composer)
    {
        ...
        if (!is_null($configExtraSettings)) {
            ...
            $mapper->push(new \BracketSpace\Notification\Signature\Dependencies\JsonMapper\Middleware\CaseConversion(
                \BracketSpace\Notification\Signature\Dependencies\JsonMapper\Enums\TextNotation::UNDERSCORE(),
                \BracketSpace\Notification\Signature\Dependencies\JsonMapper\Enums\TextNotation::CAMEL_CASE()
            ));
        ...
        if (empty($this->packages)) {
            $this->packages = array_map(function (\BracketSpace\Notification\Signature\Dependencies\Composer\Package\Link $element) {
