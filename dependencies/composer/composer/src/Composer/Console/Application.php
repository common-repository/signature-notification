...
namespace BracketSpace\Notification\Signature\Dependencies\Composer\Console;

use BracketSpace\Notification\Signature\Dependencies\Composer\Installer;
use BracketSpace\Notification\Signature\Dependencies\Composer\IO\NullIO;
use BracketSpace\Notification\Signature\Dependencies\Composer\Util\Filesystem;
use BracketSpace\Notification\Signature\Dependencies\Composer\Util\Platform;
use BracketSpace\Notification\Signature\Dependencies\Composer\Util\Silencer;
use LogicException;
use RuntimeException;
use BracketSpace\Notification\Signature\Dependencies\Seld\Signal\SignalHandler;
...
use BracketSpace\Notification\Signature\Dependencies\Seld\JsonLint\ParsingException;
use BracketSpace\Notification\Signature\Dependencies\Composer\Command;
use BracketSpace\Notification\Signature\Dependencies\Composer\Composer;
use BracketSpace\Notification\Signature\Dependencies\Composer\Factory;
...
use BracketSpace\Notification\Signature\Dependencies\Composer\IO\ConsoleIO;
use BracketSpace\Notification\Signature\Dependencies\Composer\Json\JsonValidationException;
use BracketSpace\Notification\Signature\Dependencies\Composer\Util\ErrorHandler;
use BracketSpace\Notification\Signature\Dependencies\Composer\Util\HttpDownloader;
use BracketSpace\Notification\Signature\Dependencies\Composer\EventDispatcher\ScriptExecutionException;
use BracketSpace\Notification\Signature\Dependencies\Composer\Exception\NoSslException;
use BracketSpace\Notification\Signature\Dependencies\Composer\XdebugHandler\XdebugHandler;
use BracketSpace\Notification\Signature\Dependencies\Symfony\Component\Process\Exception\ProcessTimedOutException;
use BracketSpace\Notification\Signature\Dependencies\Composer\Util\Http\ProxyManager;
...
';
    ...
    public function __construct(string $name = 'BracketSpace\Notification\Signature\Dependencies\Composer', string $version = '')
    {
            ...
            if (is_file($file) && Filesystem::isReadable($file) && is_array($composer = json_decode(file_get_contents($file), true))) {
                if (isset($composer['scripts']) && is_array($composer['scripts'])) {
                    foreach ($composer['scripts'] as $script => $dummy) {
                        if (!defined('BracketSpace\Notification\Signature\Dependencies\Composer\Script\ScriptEvents::'.str_replace('-', '_', strtoupper($script)))) {
                            if ($this->has($script)) {
     ...
     * @return \BracketSpace\Notification\Signature\Dependencies\Symfony\Component\Console\Command\Command[]
     */
    ...
    private function getPluginCommands(): array
    {
        ...
        if (null !== $composer) {
            ...
            foreach ($pm->getPluginCapabilities('BracketSpace\Notification\Signature\Dependencies\Composer\Plugin\Capability\CommandProvider', ['composer' => $composer, 'io' => $this->io]) as $capability) {
